...
export default function CampaignTable({ campaigns, loading }: CampaignTableProps) {
  ...
  const handleStatusChange = (id: number, status: string) => {
    updateCampaignMutation.mutate({
      ...
      data: {
        status,
        startedAt: status === 'active' ? new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' }) : undefined
      }
    });
  };
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          Created {formatIndianDate(campaign.createdAt)} at {formatIndianDateTime(campaign.createdAt).split(' ').slice(1).join(' ')}
                        </p>
                      </div>
                    </td>
